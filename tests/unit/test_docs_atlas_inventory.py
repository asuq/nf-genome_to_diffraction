...
def test_node_focus_opens_dark_reserved_documentation_panel() -> None:
    ...
    assert (
        scientist.index('class="header"')
        < scientist.index('class="atlas-legend-row no-print"')
    ...
    assert "First-Copy Screen + A Review" in scientist
    assert "Matthews expectation → one-copy MR → review" in scientist
