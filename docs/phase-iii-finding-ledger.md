...
| `PH3-P1-02` B registry restricted by A cap | Fixed on `dev/phase3`; real execution pending | A valid B model outside a 25-item A shortlist is schedulable by the schema-v2 planner. Every parent/candidate-copy model resolves through the checksum-verified all-model registry or retains a typed unsearchable reason; cap mutations leave the registry unchanged, while registry/source mutations change the plan identity. |
