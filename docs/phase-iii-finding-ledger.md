...
| `PH3-P1-14` Review decisions could bind a hand-written, under-specified package | Fixed locally; trusted-run/HPC wiring pending | The schema-v2 crystallographic/A-seed package now content-binds one parent run/profile/phase, Phase III execution identity, crystal, complete target set, explicit evidence checksum/size allow-list, and generated target-table coverage. The local unknown-screen stub consumes three independently staged single-crystal packages and carries only the matching stage in each crystal item. The generator rejects mutation, duplicate roles/paths, symlinks, escape, unlisted files, and non-empty publication targets; trusted owned-run wiring and composition/sequence package generation remain deferred. |
