...
| `PH3-P1-14` Review decisions could bind a hand-written, under-specified package | Fixed locally; workflow wiring pending | The schema-v2 crystallographic/A-seed package now content-binds one parent run/profile/phase, Phase III execution identity, crystal, complete target set, explicit evidence checksum/size allow-list, and generated target-table coverage. The generator rejects mutation, duplicate roles/paths, symlinks, escape, unlisted files, and non-empty publication targets; composition/sequence package generation remains deferred. |
