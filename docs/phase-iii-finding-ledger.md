...
| `PIPE-P1-08` | Partial; fixed-parent uncertainty preserved, coordinate derivation blocked safely | Fixed-A partner searches retain the reviewed first-copy identity/error source. The schema-v2 expansion input requires distinct component-only coordinates and rejects collapsed parents. The exact 6RTZ/3U7Q audit found no retained `.sol` or documented per-`SOLU 6DIM` PDBs, so a content-addressed boundary forbids chain-order inference, guessed transforms, coordinates, or commands. A future installed-runtime adapter must retain `XYZOUT ON ENSEMBLE ON` outputs, map documented file ordinals to exact `.sol` entries, preserve components from their introduction stage, and verify recombination. Multi-fixed syntax qualification, command/parser execution, and execution-to-assessment integration remain pending. |
