...
For a recorded terminal Nextflow failure, the same reviewed `logs` operation
derives the failed task work directory from Nextflow's final complete
`Work dir:` record. It accepts only owned run and cache directories with the
expected canonical identity, permitting Raven's startup-validated site-root
mount alias but no aliases inside the run/cache. The task must remain below
the fixed `cache/*/work/HH/HASH` tree; callers cannot supply a path.
The combined application/command tail is capped at 2 MiB remotely and
again after local decoding.
The normal `collect` operation adds only `.command.sh`, `.command.run`,
`.command.log`, `.command.out`, `.command.err`, `.command.trace`, and
`.exitcode` from that validated directory. Optional diagnostic files above the
2 MiB diagnostic cap or the remaining collection budget are omitted without
blocking core evidence and are recorded in a small omission manifest.
Task/cache symlinks, escaped paths, foreign ownership, unreadable files, and unlisted
siblings are rejected or omitted. This is the complete supported
...
## M4 comparative sequential-copy profile

`m4-copy` advances every explicitly approved, coordinate-bearing P2-diverse
solution through bounded one-at-a-time same-component Phaser searches. It is a
...
Each candidate starts by fixing its approved parent at the origin and searching
copy two. A packed supported child then becomes the checksum-authenticated fixed
parent for copy three, continuing one copy at a time until the candidate's
expected count or its first unsupported addition. All parents, attempts, and raw
metrics are retained. A stopped series does not prove that the component or
another copy is absent and does not remove that candidate from review. The run
has no Phaser command deadline, uses the conservative site scheduler margin,
and requires an all-cached 11-process resume before qualification succeeds.
