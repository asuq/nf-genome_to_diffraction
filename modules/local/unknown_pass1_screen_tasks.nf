...
// MTZ or model bytes, the complete inventory, global execution identity,
// staged crystallographic review, and the same three shared preparations.
...
process MATERIALISE_UNKNOWN_PASS1_CRYSTAL_ITEM {
    ...
    cp '${item[4]}' '${outputName}/unknown_pass1_screen_inventory.json'
    cp '${item[5]}' '${outputName}/phase3_execution_identity.json'
    cp -R '${item[6]}' '${outputName}/crystallographic_review_stage'
    cp -R '${item[7]}' '${outputName}/shared_catalogue'
    cp -R '${item[8]}' '${outputName}/shared_provider'
    cp -R '${item[9]}' '${outputName}/shared_localisation'
    """
}
...
process STUB_UNKNOWN_PASS1_A_HYPOTHESIS {
    ...
    cp '${item[6]}' '${outputName}/unknown_pass1_screen_inventory.json'
    cp '${item[7]}' '${outputName}/phase3_execution_identity.json'
    cp -R '${item[8]}' '${outputName}/crystallographic_review_stage'
    cp -R '${item[9]}' '${outputName}/shared_catalogue'
    cp -R '${item[10]}' '${outputName}/shared_provider'
    cp -R '${item[11]}' '${outputName}/shared_localisation'
